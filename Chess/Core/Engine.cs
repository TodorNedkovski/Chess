        ...
        public void Run()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    if (this.CheckingPosition(new Coordinate(coordinate[0], coordinate[1]),
                        new Coordinate(coordinate[2], coordinate[3]))
                        && this.IsThereSameColorFigureInFront(new Coordinate(coordinate[0], coordinate[1]),
                        new Coordinate(coordinate[2], coordinate[3])))
                    {
                        this.Move(new Coordinate(coordinate[0], coordinate[1]), new Coordinate(coordinate[2], coordinate[3]));
                    }
                    else if (this.IsThereCastling(this.board.Table[coordinate[0] - 1, coordinate[1] - 1].Color))
                    {
                        if (coordinate[0] == coordinate[2] && (coordinate[1] - 2 == coordinate[3] || coordinate[1] + 2 == coordinate[3]))
                        {
                            this.Move(new Coordinate(coordinate[0], coordinate[1]), new Coordinate(coordinate[2], coordinate[3]));
                        }
                    }
