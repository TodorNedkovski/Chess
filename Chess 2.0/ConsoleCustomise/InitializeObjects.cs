...
namespace RealChess.ConsoleCustomise
{
    public class InitializeObjects
    {
        ...
        public void InitializeBorders()
        {
            ...
            for (int i = 8; i >= 1; i--)
            {
                Console.SetCursorPosition(122, top);
                Console.WriteLine(i);
                top += 4;
            }

            //Up horizontal numbers
            for (char i = (char)97; i < 105; i++)
            {
        ...
        public void InitializeSqures()
        {
            ...
            for (int top = 3; top < 35; top++)
            {
                for (int i = 0; i < 4; i++)
                {
                    Console.SetCursorPosition(119, top);
                    Console.WriteLine(" ");
                    Console.BackgroundColor = ConsoleColor.DarkGray;
                    Console.SetCursorPosition(119, top);
                    Console.WriteLine(" ");
        ...
        private void InitializeRightVerticalBorder()
        {
            for (int y = 2; y < 35; y++)
            {
                this.drawManager.Draw("|", new List<Coordinate> { new Coordinate(121, y) });
            }
        }
